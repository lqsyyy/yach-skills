...
- [ ] Reducers return new state objects (no mutation)

### Redux-Saga
- [ ] All API calls use `call` effect
- [ ] Actions dispatched via `put` effect
- [ ] `try-catch` blocks around effect logic
- [ ] `takeLatest` used where appropriate to prevent race conditions
- [ ] No missing `yield` keywords

### Ant Design 4
- [ ] `Form.useForm()` used for form management
- [ ] `initialValues` used for default form state
- [ ] Tables use `scroll` for large datasets
- [ ] Modals use `destroyOnClose` where appropriate
...
- [ ] Persistent Server/Global state uses Redux + Redux-Saga
- [ ] Local ephemeral state uses `useState`/`useReducer`
- [ ] Complex local logic uses custom hooks or `ahooks`
- [ ] Selectors are used to minimize re-renders in connected components

### File Organization

- [ ] Feature-based folder structure for larger apps
- [ ] Critical operations (Sagas, JSON.parse, Async IO) protected by `try-catch`
- [ ] No barrel files (`index.ts`) in application code
- [ ] Shared components in `_public` or generic folders
- [ ] **Dependency Rule:** No imports from `im` in `_public`, `pages`, or `plugins`
...
- [ ] `util.locale` used for all user-facing text

### Electron & Events
- [ ] All `ipcRenderer.on` listeners have `removeListener` in cleanup
- [ ] All `document.addEventListener` have `removeEventListener` in cleanup
- [ ] No direct `fs` or `child_process` calls in UI components (move to utils/services)
