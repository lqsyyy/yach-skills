...
### 1. Missing try-catch in Critical Operations
**Problem:** Unhandled exceptions can crash the application, leave the UI in a loading state, or cause silent failures.

**Mandatory for:**
- **Sagas:** Uncaught errors terminate the generator.
- **Async Functions:** Prevents unhandled promise rejections.
- **JSON Parsing:** `JSON.parse` throws on invalid input.
...
async function fetchData() {
  ...
  try {
    ...
    console.error(error);
    message.error(util.locale('common_fetch_failed'));
  } finally {
    setLoading(false);
  }
}
...
### 2. Dispatching Too Many Actions
**Problem:** Unnecessary re-renders and complex state flow.

**Recommendation:** Batch updates or use selective selectors.
...
### 2. Illegal Module Dependency
**Problem:** Violates architectural boundaries. `im` is a feature-specific module and should not be a dependency for shared/public or other generic page/plugin modules.
